...
export class GoblinStoreScraper implements Scraper {
    name = 'Goblin Store';
    private baseUrl = 'https://www.goblin-store.com';

    private categories = [
        { url: '/product-category/magic-the-gathering/', game: 'Magic: The Gathering' },
        { url: '/product-category/yu-gi-oh/', game: 'Yu-Gi-Oh!' },
        { url: '/product-category/op/', game: 'One Piece' },
        { url: '/product-category/pkm/', game: 'Pokemon' },
        { url: '/product-category/diger-kart-oyunlari/', game: 'Other' }
    ];
        ...
        while (hasMore) {
            try {
                ...
                productCards.each((_, elem) => {
                    ...
                    if (cardText.includes('tükendi') || cardText.includes('out of stock') || cardText.includes('stokta yok')) {
                        return;
                    }

                    const title = $card.find('.woocommerce-loop-product__title, .product-title, h2, h3').text().trim();
                    const link = $card.find('a.woocommerce-LoopProduct-link').attr('href');
