...
export async function GET(request: Request) {
    try {
        ...
        // Add Vercel-compatible scrapers (Fetch/Cheerio based)
        // Puppeteer scrapers are excluded as they require special setup
        if (!specificScraper || specificScraper === 'lemon') {
            scrapers.push(new LemonCardsScraper());
        }
        if (!specificScraper || specificScraper === 'gameland') {
            scrapers.push(new GamelandScraper());
        }
        ...
        for (const scraper of scrapers) {
            ...
            try {
                ...
                stats[scraper.name] = { error: 'Failed to scrape' };
            }
        }
