...
async function loadGameland() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Gameland load process...\n');

    // 1. Scrape
    const scraper = new GamelandScraper();
    ...
    // Delete old Gameland cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Gameland']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Gameland']);
    console.log(`Total Gameland cards in DB: ${countResult.rows[0].count}`);
}

loadGameland().catch(console.error);
