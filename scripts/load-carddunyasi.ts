...
import { CardDunyasiScraper } from '../lib/scraper/carddunyasi';

async function loadCardDunyasi() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Card Dunyasi load process...\n');

    // 1. Scrape
    const scraper = new CardDunyasiScraper();
    ...
    // Delete old Card Dunyasi cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Card Dunyasi']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Card Dunyasi']);
    console.log(`Total Card Dunyasi cards in DB: ${countResult.rows[0].count}`);
}

loadCardDunyasi().catch(console.error);
