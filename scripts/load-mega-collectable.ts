...
async function loadMegaCollectable() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Mega Collectable load process...\n');

    // 1. Scrape
    const scraper = new MegaCollectableScraper();
    ...
    // Delete old Mega Collectable cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Mega Collectable']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Mega Collectable']);
    console.log(`Total Mega Collectable cards in DB: ${countResult.rows[0].count}`);
}

loadMegaCollectable().catch(console.error);
