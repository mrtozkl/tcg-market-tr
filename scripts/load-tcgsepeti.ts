...
async function loadTcgSepeti() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting TcgSepeti load process...\n');

    // 1. Scrape
    const scraper = new TcgSepetiScraper();
    ...
    // Delete old TcgSepeti cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['TcgSepeti']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['TcgSepeti']);
    console.log(`Total TcgSepeti cards in DB: ${countResult.rows[0].count}`);
}

loadTcgSepeti().catch(console.error);
