...
async function loadParalelEvren() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Paralel Evren load process...\n');

    // 1. Scrape
    const scraper = new ParalelEvrenScraper();
    ...
    // Delete old Paralel Evren cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Paralel Evren']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Paralel Evren']);
    console.log(`Total Paralel Evren cards in DB: ${countResult.rows[0].count}`);
}

loadParalelEvren().catch(console.error);
