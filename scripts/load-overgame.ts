...
async function loadOvergame() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Overgame load process...\n');

    // 1. Scrape
    const scraper = new OvergameScraper();
    ...
    // Delete old Overgame cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Overgame']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Overgame']);
    console.log(`Total Overgame cards in DB: ${countResult.rows[0].count}`);
}

loadOvergame().catch(console.error);
