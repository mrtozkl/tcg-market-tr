...
async function loadOyunkurucu() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Oyunkurucu load process...\n');

    // 1. Scrape
    const scraper = new OyunkurucuScraper();
    ...
    // Delete old Oyunkurucu cards to ensure we only have in-stock ones
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Oyun Kurucu']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Oyun Kurucu']);
    console.log(`Total Oyunkurucu cards in DB: ${countResult.rows[0].count}`);
}

loadOyunkurucu().catch(console.error);
