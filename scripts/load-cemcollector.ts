...
async function loadCemcollector() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Cemcollector load process...\n');

    // 1. Scrape
    const scraper = new CemcollectorScraper();
    ...
    // Delete old Cemcollector cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Cemcollector']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Cemcollector']);
    console.log(`Total Cemcollector cards in DB: ${countResult.rows[0].count}`);
}

loadCemcollector().catch(console.error);
