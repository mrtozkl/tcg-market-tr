...
async function loadMythosCards() {
    ...
    console.log('\n🚀 Starting Mythos Cards load process...\n');

    // 1. Scrape
    const scraper = new MythosCardsScraper();
    ...
    // Delete old Mythos Cards cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Mythos Cards']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Mythos Cards']);
    console.log(`Total Mythos Cards cards in DB: ${countResult.rows[0].count}`);
}

loadMythosCards().catch(console.error);
