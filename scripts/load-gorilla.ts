...
async function loadGorilla() {
    ...
    console.log('\n🚀 Starting Gorilla Custom Cards load process...\n');

    // 1. Scrape
    const scraper = new GorillaScraper();
    ...
    // Delete old Gorilla cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Gorilla Custom Cards']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Gorilla Custom Cards']);
    console.log(`Total Gorilla cards in DB: ${countResult.rows[0].count}`);
}

loadGorilla().catch(console.error);
