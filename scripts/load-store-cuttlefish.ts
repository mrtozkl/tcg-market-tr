...
async function loadStoreCuttlefish() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Store Cuttlefish load process...\n');

    // 1. Scrape
    const scraper = new StoreCuttlefishScraper();
    ...
    // Delete old Store Cuttlefish cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Store Cuttlefish']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Store Cuttlefish']);
    console.log(`Total Store Cuttlefish cards in DB: ${countResult.rows[0].count}`);
}

loadStoreCuttlefish().catch(console.error);
