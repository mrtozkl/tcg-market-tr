...
async function loadKollektman() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Kollektman load process...\n');

    // 1. Scrape
    const scraper = new KollektmanScraper();
    ...
    // Delete old Kollektman cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Kollektman']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Kollektman']);
    console.log(`Total Kollektman cards in DB: ${countResult.rows[0].count}`);
}

loadKollektman().catch(console.error);
