...
async function loadPegasusOyuncak() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Pegasus Oyuncak load process...\n');

    // 1. Scrape
    const scraper = new PegasusOyuncakScraper();
    ...
    // Delete old Pegasus Oyuncak cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Pegasus Oyuncak']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Pegasus Oyuncak']);
    console.log(`Total Pegasus Oyuncak cards in DB: ${countResult.rows[0].count}`);
}

loadPegasusOyuncak().catch(console.error);
