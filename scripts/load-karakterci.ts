...
async function loadKarakterci() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Karakterci load process...\n');

    // 1. Scrape
    const scraper = new KarakterciScraper();
    ...
    // Delete old Karakterci cards to ensure we only have in-stock ones
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Karakterci']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Karakterci']);
    console.log(`Total Karakterci cards in DB: ${countResult.rows[0].count}`);
}

loadKarakterci().catch(console.error);
