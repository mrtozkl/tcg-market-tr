...
async function loadGoblinStore() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Goblin Store load process...\n');

    // 1. Scrape
    const scraper = new GoblinStoreScraper();
    ...
    // Delete old Goblin Store cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Goblin Store']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Goblin Store']);
    console.log(`Total Goblin Store cards in DB: ${countResult.rows[0].count}`);
}

loadGoblinStore().catch(console.error);
