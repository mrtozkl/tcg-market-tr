...
async function loadKollektit() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Kollektit load process...\n');

    // 1. Scrape
    const scraper = new KollektitScraper();
    ...
    // Delete old Kollektit cards to ensure we only have in-stock ones
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Kollektit']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Kollektit']);
    console.log(`Total Kollektit cards in DB: ${countResult.rows[0].count}`);
}

loadKollektit().catch(console.error);
