...
async function loadLenoCards() {
    ...
    // 1. Scrape
    const scraper = new LenoCardsScraper();
    ...
    // Delete old Leno Cards cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Leno Cards']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Leno Cards']);
    console.log(`Total Leno Cards cards in DB: ${countResult.rows[0].count}`);
}
