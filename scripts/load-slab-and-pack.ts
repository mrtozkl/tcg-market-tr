...
async function loadSlabAndPack() {
    // Dynamic import to ensure env vars are loaded first
    const { db } = await import('../lib/db');

    console.log('\n🚀 Starting Slab and Pack load process...\n');

    // 1. Scrape
    const scraper = new SlabAndPackScraper();
    ...
    // Delete old Slab and Pack cards
    const deleteResult = await db.query('DELETE FROM cards WHERE seller_name = $1', ['Slab and Pack']);
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM cards WHERE seller_name = $1', ['Slab and Pack']);
    console.log(`Total Slab and Pack cards in DB: ${countResult.rows[0].count}`);
}

loadSlabAndPack().catch(console.error);
